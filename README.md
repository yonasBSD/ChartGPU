...
- **Series types**: `SeriesType` is `'line' | 'area' | 'bar'`, and `series` is a discriminated union (`LineSeriesConfig | AreaSeriesConfig | BarSeriesConfig`). See [`types.ts`](src/config/types.ts).
  ...
  - **Bar**: bar series support `barWidth?: number | string`, `barGap?: number`, `barCategoryGap?: number`, `stack?: string`, and `itemStyle?: BarItemStyleConfig` (including `borderRadius`, `borderWidth`, `borderColor`). See [`types.ts`](src/config/types.ts).
